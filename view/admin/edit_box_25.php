	...
	<div class="inside">
		...
		<?php foreach ($trail AS $pos => $item) : ?>
			<option value="<?php echo $item->id ?>">#<?php echo $pos + 1?>: 
				<?php printf (__ ('Edited by %s on %s at %s', 'audit-trail'), $item->username, date (get_option ("date_format"), $item->happened_at), date (get_option ('time_format'), $item->happened_at)); ?>
			</option>
		<?php endforeach; ?>
		</select>
